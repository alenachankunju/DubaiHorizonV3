...
export default function FeaturedDestinations() {
  ...
  useEffect(() => {
    const fetchFeaturedDestinations = async () => {
      ...
      try {
        ...
        setFeatured(typedData);

      } catch (err: any) {
        console.error("Error fetching featured destinations (raw):", err.message || err);
        let errorMessage = "Failed to load featured destinations.";
  ...
  return (
    <section className="py-12">
      <h2 className="text-3xl font-headline font-semibold text-center mb-8">Featured Destinations</h2>
      ...
      {!isLoading && !error && featured.length === 0 && (
         <p className="text-center text-muted-foreground">No featured destinations available at the moment. Add some in the admin panel!</p>
      )}
